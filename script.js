...
var selectedCell, difficulty = easy[0], answer = [""];
let diffNum = 1;
...
const startGame = () => {
	...
	if (sessionStorage.getItem("cache")) {
		...
		if (diff != diffNum) {
			if (diff == 1) changeDifficulty(1);
			else if (diff == 2) changeDifficulty(2);
			else if (diff == 3) changeDifficulty(3);
		} else {
			saved = saved.split(",");
			answer = saved;
			for (i = 0; i < 81; i++) {
...
const highSelect = () => {
	...
	for (let i = 0; i < 81; i++) {
		if (parseInt(selectedCell.textContent) == answer[i]) id(i).classList.add("dup");
	}
	...
	if ((-1 < s && s < 3) || (8 < s && s < 12) || (17 < s && s < 21)) {
		for (let i = x; i < y; i++) {
			for (let j = i; j < i + 19; j += 9) {
				if (j != s) id(j).classList.add("highlighted");
			}
		}
	}
	x = 3;
...
const createBoard = (board) => {
	...
	for (let i = 0; i < 81; i++) {
		...
		if (board.charAt(i) != "-") {
			...
			cell.addEventListener("click", () => {
				if (cell.classList.contains("selected")) {
					...
					for (let i = 0; i < 81; i++) qA(".cell")[i].classList.remove("selected");
				}
		...
		if ((cell.id > 17 && cell.id < 27) || (cell.id > 44 && cell.id < 54)) cell.classList.add("bottomBorder");
		if (((cell.id + 1) % 9 == 3) || ((cell.id + 1) % 9 == 6)) cell.classList.add("rightBorder");
		if ((cell.id + 1) % 9 == 0 || (cell.id + 1) % 9 == 0) cell.classList.add("rightBorder");
		if ((cell.id + 2) % 9 == 2 || (cell.id + 2) % 9 == 2) cell.classList.add("leftBorder");
		if ((cell.id > 71 && cell.id < 81)) cell.classList.add("bottomBorder");
		if ((cell.id > -1 && cell.id < 9)) cell.classList.add("topBorder");
...
const fillAns = (str) => {
	for (let i = 0; i < str.length; i++) answer[i] = str[i];
}
const addToCache = () => {
...
const validate = () => {
	...
	else alert("Something is not right, Try again.");
}
const endGame = () => {
	sessionStorage.clear();
	for (let i = 0; i < 81; i++) id(i).classList.add("prefilled");
	alert("You Win!, GGWP!");
}
const cani = (num) => {
	let pass = 1;
